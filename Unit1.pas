...
uses UnitGame_Over, nft, UnitMakers, UnitHello;
...
begin
  ...
  begin
    ...
    StatusBar1.Panels[1].Text := 'Уничножено метеоритов: ' +
      inttostr(score div 5);
    StatusBar1.Panels[2].Text := 'Уровень корабля: ' + inttostr(Lvl_Main+1);
  end;
...
begin
  MediaPlayer1.Play;
  Lvl_Game := 1;
  Lvl_Names[0] := 'Прогулка по космосу';
  Lvl_Names[1] := 'Космическая операция';
  Lvl_Names[2] := 'Метеоритный дождь!';
  Lvl_Names[3] := 'Падение луны!!!';
...
begin
 ...
 // bitmap.TransparentColor:=cl3DDkShadow;
  Refresh;
  // корабль
